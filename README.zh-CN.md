...
`xhigh` 档一轮 **5–8 分钟**，期间没有任何输出，直到两家都跑完。成本按 grok 上报大约
**每轮 $0.01–0.04**；codex 完全不上报成本，所以真实数字更高。
...
- **加第三位评审员**——加一条含 `args` 和 `probe` 块的记录。如果那个 CLI 的输出形状和现
  有两种都不同，还要在 `scripts/verdict.mjs` 里加一个解析器。

`BOARD_HOME` 可覆盖 skill 根目录探测。`--lang zh-CN` 让评审意见用中文返回。
